...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      publicPathUrl = `/uploads/champions/${fileName}`;
    } catch (fsErr) {
      console.warn("Local disk write note:", fsErr);
    }

    const uniqueKey = "champions/" + fileName;
    ...
    try {
      ...
      console.warn("Cloudflare R2 champion upload note:", r2Err);
    }
    ...
    return NextResponse.json(
      {
        ...
        message: "Champion card image uploaded successfully",
      },
      { headers: corsHeaders }
    );
  } catch (error: any) {
    console.error("Champion upload error:", error);
    return NextResponse.json(
